...
async function main() {
    ...
    const aPost1 = await users.makePost(andrew1['_id'], 'Andrew', 'Capro', 'Dumbbells and Backpack', "../public/photos/img.png", '3 Cool Ln.') //make creates the post, and adds it to user as well, returns postId
    await users.makePost(andrew1['_id'], 'Andrew', 'Capro', 'Potted Plant', '../public/photos/img2.jpg', '400 West St.')
    const aPostID = aPost1

    //user two
    const nick1 = await users.createUser('Nick', 'Mule', 'nmule@gmail.com', 'nmule', 'Nick123!') //whole user as an object
    const nPost = await users.makePost(nick1['_id'], 'Nick', 'Mule', 'Couch', '../public/photos/img3.jpg', '800 Washington St.') //returns postId
    const nPostID = nPost
    ...
    const cPost1 = await users.makePost(casey1['_id'], 'Casey', 'Mulrooney', 'Mulch', '../public/photos/img4.jpg', '300 Bloomfield Ave.')
    await users.makePost(casey1['_id'], 'Casey', 'Mulrooney', 'White Chairs', '../public/photos/img5.jpg', 'Outside Babbio Center')
    ...
    const chPost = await users.makePost(chance1['_id'], 'Chancelor', 'Assiamah', 'Kids Bike', '../public/photos/img6.jpg', '800 Washington St.')
