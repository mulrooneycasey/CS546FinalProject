...
async function main() {
    ...
    //user one
    const andrew1 = await users.createUser('Andrew', 'Capro', 'acapro@gmail.com', 'acapro', 'Andrew123!', 22) //create returns whole user object with id as a string
    const aPost1 = await users.makePost(andrew1['_id'], 'Andrew', 'Capro', 'Dumbbells and Backpack', "../public/photos/img.png", '3 Cool Ln.', 'Dumbbells; and; Backpack') //make creates the post, and adds it to user as well, returns postId
    await users.makePost(andrew1['_id'], 'Andrew', 'Capro', 'Potted Plant', '../public/photos/img2.jpg', '400 West St.', 'Potted; Plant')
    const aPostID = aPost1

    //user two
    const nick1 = await users.createUser('Nick', 'Mule', 'nmule@gmail.com', 'nmule', 'Nick123!', 21) //whole user as an object
    ...
    //user three
    const casey1 = await users.createUser('Casey', 'Mulrooney', 'cmulrooney@gmail.com', 'cmulrooney', 'Casey123!', 21)
    ...
    //user four
    const chance1 = await users.createUser('Chancelor', 'Assiamah', 'cassiamah@gmail.com', 'cassiamah', 'Chancelor123!', 22)
    const chPost = await users.makePost(chance1['_id'], 'Chancelor', 'Assiamah', 'Kids Bike', '../public/photos/img6.jpg', '800 Washington St.', 'Kids; Bike')
